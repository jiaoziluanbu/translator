...
1. 双击 `LocalTranslator-2.2.1.dmg`（或当前最新版），把 **Local Translator.app** 拖到 Applications。
...
在菜单栏的「目标语言」中明确选择中文、英文、日文或韩文后，目标语言会保持锁定。截图或选中文字含有多种语言时，各段会分别判断源语言并统一翻译到所选目标；只有「自动」模式会继续按内容选择中译英或外语译中。
...
- **首次冷启动**：daemon 启动后会在后台预热 hot worker。预热尚未完成或 worker 空闲释放后，⌃⌥A 仍会立即显示截屏选区；较慢的 OCR/翻译组件改为在完成选区后加载，因此首次译文出现仍可能多等 2-5 秒，但不会再卡住截屏框。
